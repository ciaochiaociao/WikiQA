...
class WikiQA:
    def __init__(self):
        self.corenlp_ip = 'http://140.109.19.190:9000'
    ...
    def _predict(self, fgc_data, save_result=True):
        global q, wd_item, rel, matched_attr_names, predicate_matched, nlp_data, passage_data, mentions_bracketed, \
            ...
            for q in item['QUESTIONS']:
                ...
                for ix, ent_link_cand in enumerate(ent_link_cands):
                    ...
                    for ix, wd_item in enumerate(wd_items):
                        ...
                        # match predicates
                        rel_datavalues_matched_tuples = list(self.get_values_from_matched_predicate(wd_item, rest_text))
                    ...
                    for wd_item, rel_datavalues_matched_tuples in wd_item_rel_datavalues_matched_tuples:
                        ...
                        for rel, datavalues in rel_datavalues_matched_tuples:
                            ...
                            print(f'labels of attributes matched the question text\n' +
                                  'ent_link_query: {} | labels: {} ({}) | values: {} | answers: {}'.format(
                                      ent_link_query, matched_attr_names, predicate_matched, datavalues,
                                      answers))
                            # TODO
                            debug_info.update({
                            })
                ...
                def filter_answers(answers):
                    ...
                    for answer in answers:
                        if answer not in qtext or qtext not in answer:
                            answers.append(answer)
                    return answers

                final_answers = filter_answers(answers_from_ent_link_queries)
...
def match_ans_type(span: tuple):
    # TODO
    mention = get_ent_from_tok_b_e(passage_data.mentions, span)
    if match_type(mention, q['ATYPE']):
        return True
    return False


def match_passage_and_answer(ans_cand: str, dtext: str, nlp_data):
    ...
    if matches:
        print('All fuzzy matched answers:', [match.group(0) for match in matches])
        ...
        for match in matches:
            if len(ans_cand) > 1 and match_ans_type(match.span(0)):
                return True
    return False
...
def predicate_matching(label, rest_text):
    ...
    strict_label_map = {
        '名字': [f'(.+){what}名字', f'(.+)名字(叫|是){what}']
    }

    for pattern in strict_label_map.get(label, []):
        pattern = re.compile(pattern)
        result = pattern.match(qtext)
...
def preprocess_values(datavalues):
    ...
    for ix, datavalue in enumerate(datavalues):
        ...
        if datatype == 'wikibase-item':
            ...
            year, month, day = m.groupdict()['year'], m.groupdict()['month'], m.groupdict()['day']
            time_formats = [
                ...
                '{}年{}月'.format(m.groupdict()['year'], m.groupdict()['month']),
    ...
    if isinstance(dvalue_comp, tuple):  # if the datavalue is a wikidata item
        ...
        for (answer_match_way, ans_cand) in list(labels) + list(aliases):  # all possible labels in the wikidata item
            if ans_cand in shown:  # skip duplicate values
                continue
            shown.add(ans_cand)
            if match_passage_and_answer(ans_cand, dtext, nlp_data):
                answers_from_one_datavalue.append(ans_cand)
    else:  # if the datavalue is not a wikidata item
        if isinstance(dvalue_comp, list):
            # TODO
            pass
        elif isinstance(dvalue_comp, str):
            if match_passage_and_answer(dvalue_comp, dtext, nlp_data):
                answers_from_one_datavalue.append(dvalue_comp)
        else:
            pass
